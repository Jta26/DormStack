...
export default class SyncTree {
  ...
  _handleValueEvent(event) {
    ...
    return INTERNALS.SharedEventEmitter.emit(eventRegistrationKey, new DatabaseSnapshot(registration.ref, snapshot), previousChildName);
  }
   ...
   * @return {String}
   */
  addRegistration(parameters, listener) {
    const {
      path,
      eventType,
    ...
    if (once) {
      INTERNALS.SharedEventEmitter.once(eventRegistrationKey, this._onOnceRemoveRegistration(eventRegistrationKey, listener));
    } else {
